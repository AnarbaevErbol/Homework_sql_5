...
SELECT * FROM products WHERE  products.company not in ('HTC'); --1
SELECT * FROM products WHERE company ='Apple' AND price<75000;--2
SELECT * FROM products where price>=46000 ; --3
select productName,price from products    --4
group by productName,price
order by max(price) desc
limit 2;

select productName,price from products --5
group by productName, price
order by min(price) asc
limit 1;

SELECT * from products where price = (select min(price) from products); --5
